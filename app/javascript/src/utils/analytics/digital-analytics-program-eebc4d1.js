...
(function () {
  ...
  //*********GA4************
  const dap_head = document.getElementsByTagName("head").item(0);
  const GA4Object = document.createElement("script");
      ...
      if ("page_view" === a.toLowerCase())
        {try {
          if (Object.keys(b).length !== 0) {
            const ur = ((b.page_location) ? b.page_location : location.href);
        ...
        {try {
          const e_n = ((/^(((email|telephone|image|cta|navigation|faq|accordion|social)_)?click|file_download|view_search_results|video_(start|pause|progress|complete|play)|official_USA_site_banner_click|form_(start|submit|progress)|content_view|social_share|error|sort|filter|was_this_helpful_submit)$/gi.test(a)) ? a : 'dap_event');
  ...
  function _sendEvent(a, b) {
    let send_to = "";
    for (let g = 0; g < oCONFIG.GWT_GA4ID.length; g++) {
      try {
        send_to += oCONFIG.GA4_NAME + g + ",";
      }
      catch (er) { }
    }
    let c = _piiRedactor(_objToQuery(b), "json");
    c = _queryToJSON(c);
    c = _unflattenJSON(c);
  ...
  function createTracker(a) {
    ...
    let c = m.split(document.location.hostname)[1];
    -1 !== document.title.search(/404|not found/i) &&
      (c = ("/vpv404/" + c).replace(/\/\//g, "/") + ((document.referrer) ? "/" + document.referrer : document.referrer));
    const p = ((-1 !== document.title.search(/404|not found/ig)) ? document.location.protocol + "//" + document.location.hostname + c : m);
    const ur = _URIHandler(_scrubbedURL(p));
    let r = {};
    for (let b = 0; b < oCONFIG.GWT_GA4ID.length; b++) {
      if (b === 0) {
        r = {
  ...
  function _initAutoTracker() {
     ...
     */
    const _isDownload = function (a) {
      const ex = a.href.toLowerCase().replace(/[#?&].*/, '').split(a.hostname)[1].split("."); const ext = ex[ex.length - 1];
 ...
 */
  ...
  if (oCONFIG.YOUTUBE) {
    ...
    const _milestoneController = oCONFIG.YT_MILESTONE;
    const ytUtils = [];
    ...
    onPlayerStateChange = function (event) {
      try {
        let videoIndex = 0, video_id = ((event.target.playerInfo !== undefined) ? event.target.playerInfo.videoData.video_id : event.target.getVideoData().video_id);
        for (let o = 0; o < videoArray.length; o++) {
          if (videoArray[o] == video_id) {
            videoIndex = o;
          }
        }
        const cTime = ((playerArray[videoIndex].playerInfo !== undefined) ? Math.round(playerArray[videoIndex].playerInfo.currentTime) : Math.round(playerArray[videoIndex].getCurrentTime()));
        const vDuration = ((playerArray[videoIndex].playerInfo !== undefined) ? Math.round(playerArray[videoIndex].playerInfo.duration) : Math.round(playerArray[videoIndex].getDuration()));
        const p = {
        ...
        if (event.data == YT.PlayerState.PLAYING && p.video_percent == 0) {
          ...
          if (_milestoneController) {
            ytUtils.push([videoIndex, function (videx) {
              for (let b = 1; b <= (100 / _milestoneController); b++) {
              ...
              setInterval(() => {
                const cTimeP = ((playerArray[videoIndex].playerInfo !== undefined) ? Math.round(playerArray[videoIndex].playerInfo.currentTime) : Math.round(playerArray[videoIndex].getCurrentTime()));
                const vDurationP = ((playerArray[videoIndex].playerInfo !== undefined) ? Math.round(playerArray[videoIndex].playerInfo.duration) : Math.round(playerArray[videoIndex].getDuration()));
                const y = {
     ...
     */
    youtube_parser = function (e) { const t = e.match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/); if (t && 11 == t[2].length) {return t[2]} };
    /**
     * @param {string} u a URL
     * @returns {boolean} true if the URL is a YouTube video URL, false otherwise.
     */
    IsYouTube = function (u) { const e = u.match(/(.*)(youtu\.be\/|youtube(\-nocookie)?\.([A-Za-z]{2,4}|[A-Za-z]{2,3}\.[A-Za-z]{2})\/)(watch|embed\/|vi?\/)?(\?vi?=)?([^#&\?\/]{11}).*/); return null != e && e.length > 0 };
    /**
    ...
    _initYouTubeTracker = function () {
      ...
      for (let iframe = 0; iframe < allIframes.length; iframe++) {
        const video = allIframes[iframe];
        const _thisSrc = video.src;
        if (IsYouTube(_thisSrc)) {
          allIframes[iframe].src = YTUrlHandler(_thisSrc);
          const youtubeid = youtube_parser(_thisSrc);
  ...
  function _initHTMLVideoTracker() {
    const _milestone = oCONFIG.YT_MILESTONE;
    const media_status = {};
    /**
    ...
    function eventHandler(e) {
      const media_type = ((e.target.nodeName === 'VIDEO') ? 'video' : 'audio');
      let mObj = {};
      ...
      switch (e.type) {
        case 'timeupdate':
          ...
          for (const j in media_status[e.target.id]._progress_milestones) {
        ...
        case 'ended':
          ...
          for (let b = 1; b <= (100 / _milestone); b++) {
    ...
    const htmlMedia = document.querySelectorAll('video,audio');

    for (let i = 0; i < htmlMedia.length; i++) {
      var mediaTagId;
  ...
  function _payloadInterceptor() {
    ...
    if (!window._isRedacted) {
      ...
      try {
        ...
        const ga4_props = oCONFIG.GWT_GA4ID.join("|");
        /**
        ...
        window.navigator.sendBeacon = function () {
          if (arguments && arguments[0].match(/google-analytics\.com.*v\=2\&/i) && arguments[0].match(new RegExp(ga4_props))) {
            const endpoint = arguments[0].split('?')[0], query = arguments[0].split('?')[1];
            const beacon = {
              endpoint, query: _piiRedactor(query, "ga4"), events: []
            };
            if (arguments[1]) {
              arguments[1].split("\r\n").forEach((event) => {
  ...
  function _piiRedactor(payload, type) {
    try {
      const checkParams = "dl|dr|dt|dt|en|ep.|up.|uid";
      const UncheckParams = "ep.agency||ep.subagency|ep.site_topic|ep.site_platform|ep.script_source|ep.version|ep.protocol";
      const piiRegex = _piiRegexReset();
      payload = (("object" === typeof payload && /json|default/.test(type)) ? (_flattenJSON(payload), payload = _objToQuery(payload)) : payload);
      _piiRegexReset();

      const _allowedQs = _allowedQuerystrings.toString().toLowerCase().replace(/\,/g, "=|") + "=";
      const _hitPayloadParts = payload.split('&');
      for (let i = 0; i < _hitPayloadParts.length; i++) {
        let newQueryString = '';
        const _param = _hitPayloadParts[i].split('=');
        const _para = (_param.length > 2) ? _param.slice(1).join("=") : _param[1]; _param.splice(2); _param[1] = _para;
        var _val;
        try {
        ...
        if ((_param[0].match(new RegExp(checkParams)) != null || /query|json/ig.test(type)) && _val.indexOf('?') > -1) {
          let paramArray = _val.split('?').splice(1).join('&').split('&');
          const paramSubArray = [];
          ...
          for (let ix = 0; ix < paramArray.length; ix++) {
  ...
  function _initBannerTracker() {
    try {
      const acord = document.querySelector('section.usa-banner button.usa-accordion__button');
      if (acord) {
  ...
  function _URIHandler(a) {
    const b = new RegExp("([?&])(" + oCONFIG.SEARCH_PARAMS + ")(=[^&]+)", "i");
  ...
  function _scrubbedURL(z) {
    ...
    let n = new RegExp(`^(https?:\\/\\/(www\\.)?)?${RegExp.escape(document.location.hostname.replace(/^www\\./, ""))}`, "ig"),
      t = "",
